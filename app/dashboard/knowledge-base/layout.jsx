    ...
    <div className="relative flex max-h-screen bg-white text-gray-900">
      ...
      >
        <div className={`transition-opacity duration-150 flex flex-col min-h-0`}>
          <div className="p-4">
            {!isCollapsed ? (
              <button onClick={() => setShowCreate(true)} className="w-full bg-black text-white font-medium py-2.5 rounded-lg hover:bg-gray-800 transition flex items-center justify-center">
                <Plus size={16} className="mr-2" />
                New Folder
              </button>
            ) : (
              <button onClick={() => setShowCreate(true)} className="w-12 h-12 bg-black rounded-md flex items-center justify-center mx-auto">
              ...
              filtered.map((folder) => {
                ...
                return (
                  <div key={folder.folder_id} className="px-2 py-3 flex justify-center">
                    <Link href={`/dashboard/knowledge-base/${folder.folder_id}`} className="w-10 h-10 rounded-md bg-black flex items-center justify-center text-white text-sm font-semibold">
                      {initial}
                    </Link>
                  </div>
                );
      ...
      <button type="button" onClick={() => setIsCollapsed((s) => !s)} aria-label={isCollapsed ? "Open sidebar" : "Close sidebar"} style={{ left: isCollapsed ? 0 : expandedHandleLeft - Math.floor(handleSize / 2), width: handleSize, height: handleSize }} className="absolute top-1/2 -translate-y-1/2 z-50 rounded-full bg-black border border-gray-200 shadow-sm flex items-center justify-center hover:bg-black/80 cursor-pointer">
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
          <div className="bg-white rounded-lg shadow p-6 w-full max-w-md">
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <button onClick={submitRename} className="px-4 py-2 bg-black text-white rounded disabled:opacity-60" disabled={actionLoadingFor === renameModalFor}>{actionLoadingFor === renameModalFor ? "Updating..." : "Update"}</button>
            </div>
          </div>
        </div>
      )}
