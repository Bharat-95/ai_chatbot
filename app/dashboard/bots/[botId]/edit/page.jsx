      ...
      <div className="max-w-5xl mx-auto">
        ...
        <section className="mb-10">
          ...
          <label className="block mb-6">
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Give this bot a name" className="w-full rounded-lg border border-gray-200 px-4 py-3 placeholder:text-gray-400 focus:outline-none focus:ring-0 focus:ring-black" style={{ boxShadow: "none", borderColor: "#e6e9ee" }} />
          </label>
          ...
          <label className="block mb-6">
            ...
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={4} className="w-full rounded-lg border border-gray-200 px-4 py-3 placeholder:text-gray-400 focus:outline-none focus:ring-0 focus:ring-black" style={{ borderColor: "#e6e9ee" }} />
          </label>
                ...
                <div className="space-y-2 max-h-72 overflow-auto">
                  ...
                  {foldersToShow.map((f) => (
                    <div key={f.folder_id} className="flex items-center justify-between border border-gray-200 rounded-md p-3">
                      <div className="flex items-center gap-3">
                        <button onClick={() => toggleSelect(f)} className={`w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center ${isSelected(f.folder_id) ? "bg-black text-white border-black" : "text-black border-gray-200"}`}>{isSelected(f.folder_id) ? "✓" : "+"}</button>
                        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded bg-black text-white disabled:opacity-60">{saving ? "Saving..." : "Save changes"}</button>
        </div>
      </div>
    </div>
  );
